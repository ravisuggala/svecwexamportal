...
if(!midno.equals("final")){
	 subjects=new SubjectDB().getAllMappedSubjectsInOrderNoLabs(branch, sem, reg, section, ay);
	marksdata=medb.getAllMidMarks(ay, branch, reg, sem,  midno,section);
}
	// marksdata=medb.getElectiveMidMarks(ay, branch, reg, sem,  midno,section);
else
{
	 subjects=new SubjectDB().getAllMappedSubjectsInOrder(branch, sem, reg, section, ay);
	marksdata=medb.getAllFinalMarks(ay, branch, reg, sem,  midno,section);
}
		  ...
		  <div id="centre">
			    ...
			    <TR>
				   <TD COLSPAN=<%=colspan %> ALIGN="center"><h6><b> Student List</b></h6></TD>
						  ...
						  <INPUT TYPE=HIDDEN NAME="option" VALUE = "xlallmidmarks">
						 <input id="btnSubmit" type="button" value="Export to Excel" onClick=selectOption("xlallmidmarks") ></input>
